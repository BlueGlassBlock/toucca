            ...
            Self::Absolute(ranges) => {
                ...
                for (i, (st, end)) in ranges.iter().enumerate() {
                    if *st <= ring && ring <= *end {
                        ...
                        debug!("{} {} -> {}", section, i, res.last().unwrap());
                    }
                }
                res
            }
            ...
            0 => {
                ...
                for (i, range) in ranges.iter_mut().enumerate() {
                    *range = (divisions - 4 + i, divisions - 4 + i);
                    ...
                    debug!("Set ring {} touch range: {} - {}", i, range.0, range.1);
                }
...
// from segatools
pub const MERCURY_IO_DEFAULT_CELLS: [VIRTUAL_KEY; 240] = [
    // 1234567890
